...
Re-measured 2026-09-04 for the Machined Console ground change (T1,
hardware-console run; WCAG relative-luminance math over the actual
`color-mix(in srgb, …)` composites — same method as every prior audit;
prior-ground #0d0b10 values noted inline where history matters). Every
pair still passes with margin; the ink-on-ground ≥16:1 target holds.

Ink-mix notation: `ink@X%` = `color-mix(ink X%, ground)`.

| Pair (selector → effective colors)                                                                                     | Ratio           | AA   | Action                                                                                                                                                                                                                                                                                      |
| ---------------------------------------------------------------------------------------------------------------------- | --------------- | ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ink on ground (all primary text: buttons, values, labels, readouts)                                                    | 16.93:1         | PASS | — (was 17.49:1 on ground #0d0b10; ≥16:1 target holds)                                                                                                                                                                                                                                       |
| ink@78% row labels                                                                                                     | 10.44:1         | PASS | — (was 10.69:1)                                                                                                                                                                                                                                                                             |
| ink@62% head-ctl-labels, projects-when, stage-hint (FIXED from 45%)                                                    | 6.92:1          | PASS | FIXED (hint was 4.15:1). DA-3 note: the DA-2 axe gate's page never loaded the token sheet, so it measured this pair against a white page (1.07:1 false negative elsewhere). The gate now imports `base.css`, so contrast pairs are computed from the real rendered palette; measured 6.98:1 |
| ink@60% booth labels, lane count                                                                                       | 6.58:1          | PASS | — (was 6.61:1)                                                                                                                                                                                                                                                                              |
| ink@55% dim-legends: fx-empty/cap, rail tile bars/cue-dash (FIXED from 30%), fill tag                                  | 5.65:1          | PASS | FIXED (cue dash was 2.46:1)                                                                                                                                                                                                                                                                 |
| rail tile bars on STATE tiles (selected/active/pending lane-hue fills) — full ink (FIXED in DA-3 from ink@55%)         | 8.16–14.11:1    | PASS | FIXED (ink@55% over the lane-hue fills measured 3.12–4.14:1); range spans active 18% / selected 28% / pending-hatch 32% fills on all four hues (was 8.5–12.7:1)                                                                                                                             |
| ink@55% bypassed fx module name (FIXED from 45%) + param labels (FIXED from 40%)                                       | 5.65:1          | PASS | FIXED (were 4.15 / 3.51)                                                                                                                                                                                                                                                                    |
| Disabled buttons: fx-mod-btn (0.35→0.55), rail-tool / fill-apply SET (0.4→0.55)                                        | 5.65:1          | PASS | FIXED (were 2.94–3.51; WCAG exempts disabled, we pass anyway)                                                                                                                                                                                                                               |
| ink-on-fill on full lane fills (on-cells, rail/booth active states)                                                    | 5.17–10.91:1    | PASS | — (unchanged: neither the fills nor ink-on-fill moved)                                                                                                                                                                                                                                      |
...
| Toast sub-copy (ink@85%) / banner body (@90%)                                                                          | 12.31 / 13.71:1 | PASS | — (was 12.63 / 14.14:1)                                                                                                                                                                                                                                                                     |
| Save indicator error state (@55% opacity)                                                                              | 5.65:1          | PASS | —                                                                                                                                                                                                                                                                                           |
| Scale popover detach dimmed (@55%)                                                                                     | 5.65:1          | PASS | —                                                                                                                                                                                                                                                                                           |
| Focus indicator: 2px warm-white outline on every interactive (3:1 UI floor)                                            | 16.93:1         | PASS | — (tracks ink-on-ground)                                                                                                                                                                                                                                                                    |
| Lane-hue borders/fills vs ground (SC 1.4.11)                                                                           | 5.01–10.56:1    | PASS | — (was 5.17–10.91:1; drums 5.01:1 still clears the 3:1 UI floor and its constrained ≤10px Silkscreen-700 text role)                                                                                                                                                                         |
| Help overlay fallback ink #e8e2d4 on ground                                                                            | 14.67:1         | PASS | — (was 15.16:1)                                                                                                                                                                                                                                                                             |
| Metal chassis materials (Machined Console, for the T2+ restyles): ink on --metal-panel / --metal-raised / --metal-recess | 15.62 / 14.22 / 17.74:1 | PASS | NEW (T1): panel-label legibility probes for the token family — every metal surface clears ink AA by >3×                                                                                                                      |
...
Brush-grain interaction note (T4/M1, 2026-09-04): the Machined Console brush
grain (`--metal-brush`/`--metal-brush-v`, 1px-on/1px-off stripes) was raised
0.02 → 0.045 alpha at the M1 checkpoint's AC-9 round 2 (the material was
sub-visible in static review). The table above measures the flat
color-mix composites; text over a striped panel sees, per WCAG's anti-aliased
averaging, the stripe-MEAN composite — re-derived worst pairs (drums floor,
the strongest tint): ink@62% **6.13:1** (worst-case full-stroke-on-stripe
5.75), ink@55% **5.10** (worst 4.78); booth panel ink@62% **6.31** (worst
5.93); ink on deck **16.18:1** (worst 15.35 — the ≥16 target holds at the
mean; the deck shows only in gaps, no primary text sits on it). Every pair
≥ AA 4.5:1 in the absolute worst case. Recess wells and raised keys carry
no grain (recess/raised surfaces are flat by material law), so those pairs
are unchanged.
...
semantic surfaces: per-lane OCT register controls, register-window
scroll, pattern resize with refuse-on-loss, the changed rail `+`, and
per-lane position at unequal cycle lengths. Same contract as §7: each
extension is a DoD item for its owning task — the task implements the
semantics AND lands the assertion; until then §6 stays unchanged law. The
keyboard equivalents live in `docs/dev/keyboard.md` §"v3 delta" (pointer +
keyboard both required, identical announcements — the E5 law applies to
every row below). Status: ALL PENDING (spec'd by KL-1 2026-09-04; none
implemented yet).

| #   | Extension (semantics required)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Owning task | Gate assertion when it lands                                                                                                                                                                                                                                                                                                                                                                   |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| E8  | **OCT register controls speak value + clamp.** The OCT − / OCT + buttons and register readout live in the always-operable quadrant-strip row (all four quadrants); the readout exposes the current octave as TEXT at all times (an `aria-live=polite` value span — the stepper pattern; a slider-widget readout MUST carry `aria-valuetext` with the same signed text, e.g. "OCTAVE +1"). Every transpose — button click, global `o`/Shift+`o`, pointer — announces identically: `LEAD OCTAVE +1`; at the −3..+3 domain clamp the no-op still announces (`LEAD OCTAVE +3 · AT LIMIT`); drums answers `DRUMS HAS NO OCTAVE`. Never a silent no-op. | RC-1        | Browser journey: drive OCT by key AND by click per lane → the value-span texts are asserted equal between paths, clamp texts fire at ±3, the readout text is present before any interaction; undo restores (one gesture per held burst).                                                                                                                                                        |
| E9  | **View-scroll vs transpose is distinguishable in SR text (the Professor X conflation fence).** Register-window scrolling is VIEW-ONLY: Shift+↑/↓ on a focused cell announce `VIEW DOWN ONE OCTAVE · ROWS <a>–<b>` (+ `VIEW AT TOP/BOTTOM` at clamps) and NEVER write the document, audition, or emit octave wording; OCT changes announce `OCTAVE <signed>` and never scroll the window. Pitched grid names carry the visible window (`<LANE> grid · EDITING · ROWS 8–14 OF 14`); arrow nav walks the full manifest with the window following focus (focus always visible — the E2 law extended); the focus-anchor law bounds the scroll keys. | RC-1        | Browser journey: scroll the window by keys → announcement says VIEW, document bytes unchanged, focus unmoved; transpose by keys → announcement says OCTAVE, window unmoved; walk arrows past the window edge → window scrolls minimally, focused row stays visible; grid names carry the range and flip with the window.                                                                      |
| E10  | **Resize announces, refusal names the loss.** Pattern LENGTH changes announce `PATTERN <L> · <n> BARS`; a shrink that would lose any note past the new end is REFUSED (never silently truncated) and the announcement names the blocking note deterministically (greatest end, ties → latest start): `CANNOT SHRINK PATTERN B TO 4 BARS · <ROW> NOTE AT BAR 5 WOULD BE LOST · MOVE OR SHORTEN IT FIRST`. Menu stepper, global `b`/Shift+`b`, and pointer produce the SAME text through one funnel; after a successful resize, grid focus lands on the carried cell (step clamped to the new extent) or stays put if it was outside the grid. | LL-1 (edges HL-1) | Browser journey: grow + clean-shrink by keys and by menu → success texts + undo; a lossy shrink by BOTH paths → identical refusal text, document unchanged; focus-carry after remount asserted at the old focused step ≤ new extent and clamped at it.                                                                                                                                      |
| E11  | **Clip creation is announced, duplication has exactly one path.** The rail `+` (button and rail-local key) creates a NEW blank next-letter pattern and announces `PATTERN B CREATED · 1 BAR · APPENDED` through the lane's rail status region; the control's accessible name + help text say NEW (stale "append" wording fails the HP-2 coverage gate). DUP (PAT menu + global `d`) is unchanged and is the ONLY duplicator — no second duplication path may appear. | BC-1        | Browser journey: `+` by key and by click → creation announcement + new tile label + selection flips to the blank pattern; DUP still duplicates with pool +1; grep-level registry check: no control duplicates-by-`+` anymore.                                                                                                                                                                  |
| E12  | **Position stays speakable at unequal cycle lengths.** The booth readout + beat announcements wrap at the FULL LCM cycle (one stable global reference; byte-identical while lanes share one length — the compat law); lane wraps ride the EXISTING per-lane rail status regions (no new global wrap announcements — the spam fence); the on-demand `p` key announces `POSITION BAR 12 OF 64 · BASS BAR 4 OF 4` (lane half omitted at equal lengths) through the stage status region. | LL-2 (live) | Browser journey: mixed-length chains playing → `p` at a known transport time reads the correct global + active-lane positions; per-lane switch announcements fire at each lane's own rate; booth readout spans the LCM cycle; equal-length project → readout byte-identical to the v0.1 basis (the zero-drift proof).                                                                        |

Contrast/motion carry-over for the new surfaces (law applies now, asserted
with each task): the OCT value/clamp texts and window-range names are ink
text on the ground or lane-hue fills (the §5 pairs); the register window
moves as a discrete scroll (no transition — or it needs the §4
reduced-motion gate); the refusal/creation texts ride the existing
status-region styling. No new motion is spec'd anywhere in the v3 delta.
