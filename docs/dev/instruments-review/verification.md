...
Added-instrument titles derive from the selected preset category, matching the preset picker (Bells, Brass, Keys, Leads, Bass, Pads & chords, Plucked strings, Sound effects). Headers, phone tabs, Song labels and the meter bank update on preset changes; stable lane ids remain unchanged. Verified category changes across all four views and reload persistence with scripts/verify-category-titles.mjs. TypeScript, scoped lint and production build passed.

## Eight-lane visualizer — 2026-09-12

Visualizer composition settings and bounded activity ledgers now support eight lanes. Existing v2 four-lane saved compositions retain their original layers and receive defaults for missing extra layers. Invalid supplied extra-layer data is still rejected. The MIDI normalization path accepts all lane ids. VizPage routes mix gains by document lane identity, including sparse extra slots, and marks absent lanes inaudible. Dynamic inspector tabs show only existing tracks using their category names and colors; mobile uses two rows of four for eight tracks. Reduced-motion activity summaries include extra-track activity.

Validation: 141 focused unit tests pass across composition, clamps, pipeline, offset queue and reduced-motion coverage. Added compatibility and independent extra-voice/clear tests. TypeScript, scoped ESLint and build pass. Browser played Glass Arcade, confirmed finite audible note activity for every extra layer and inspected rendered output on desktop and phone. All eight selectors fit; changing extra4 to Woven light persisted after reload and reopening the demo. No page errors. Browser probe modules are preloaded before playback to avoid dev HMR module replacement while polling.
