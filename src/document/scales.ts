...
export function effectiveScale(project: ProjectDocument, laneId: LaneId): EffectiveScale {
...
export function toEffectiveScale(config: ScaleConfig): EffectiveScale {
  return { root: config.root, mode: config.mode, intervals: MODE_INTERVALS[config.mode] };
}
 ...
 */
export function degreeToMidi(scale: EffectiveScale, degree: number, octaveBase: number): number {
  const size = scale.intervals.length;
  const octave = Math.floor(degree / size);
  const index = ((degree % size) + size) % size;
  return 12 * (octaveBase + 1) + scale.root + scale.intervals[index] + 12 * octave;
}
...
export function chordRows(scale: EffectiveScale, octaveBase: number): ChordRow[] {
  const rows: ChordRow[] = [];
  for (let i = 0; i < scale.intervals.length; i++) {
    ...
    rows.push({
      degree: i,
      degrees,
      midi: degrees.map((d) => degreeToMidi(scale, d, octaveBase)) as [number, number, number],
    });
  }
  return rows;
}
