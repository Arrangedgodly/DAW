...
Approved pitch orientation: all pitched grids show higher MIDI pitches above lower
pitches. Display order does not rewrite stored degrees. `pitchWindow.ts` derives
scale-only rows across MIDI 0 through 127 independently of historical row lists.
Negative and high degrees are valid in the schema envelope -128 through 128 when
placement resolves within physical MIDI bounds. Browsing never transposes music.

Implementation record: `src/styles/themes.css` defines Vector dark and Alloy light,
3px panels, 4px inset displays, locally hosted Barlow Bold 23px instrument headings and Segoe UI utility
text. `ThemeSelector.tsx` lives in Booth; `bitbounce.theme.v1` persists the device
choice outside project data. Phone `fitPhoneRows` bounds available height by the
viewport, reserves following siblings including the footer, and retains a 44px base
row floor. Compact phone header/register gaps preserve space for complete rows.
