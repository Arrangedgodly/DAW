...
describe("project file import (real codec + IndexedDB + component)", () => {
  it.each([false, true])(
    "imports a file and retargets autosave (existing saved project: %s)",
    async (saved) => {
      const db = await freshDb(`bitbounce-test-fileio-ok-${saved}`);
      const before = docStore.getState().doc;
      const boot = await initPersistence({ db });
      if (saved) {
        setProjectName("Existing saved song");
        await getAutosaveController()!.flush();
      }
      const savedBefore = await db.allRecords();
      ...
      try {
        ...
        // The untouched welcome preview has no row. Only the import is saved.
        const rows = await db.allRecords();
        expect(rows).toHaveLength(savedBefore.length + 1);
        for (const existing of savedBefore)
          expect(rows).toContainEqual(existing);
        const importedRow = rows.find(
          (r) => r.name === "friend song (imported)",
        );
  ...
  it.each([false, true])(
    "rejects corrupt files without changing projects (existing saved project: %s)",
    async (saved) => {
      const db = await freshDb(`bitbounce-test-fileio-corrupt-${saved}`);
      ...
      try {
        ...
        expect(await db.allRecords()).toEqual(savedBefore);
