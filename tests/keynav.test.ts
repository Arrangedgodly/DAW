...
describe("gridMoveForKey", () => {
  ...
  it("returns null for everything else (never hijacks unrelated keys)", () => {
    for (const key of ["Tab", "Enter", " ", "Escape", "x", "F2", "Delete", "[", "]"]) {
...
describe("nextCell (bounds + no-wrap law)", () => {
  it("moves one step / one row", () => {
    expect(nextCell({ row: 2, step: 3 }, DIMS, "right")).toEqual({ row: 2, step: 4 });
    expect(nextCell({ row: 2, step: 3 }, DIMS, "left")).toEqual({ row: 2, step: 2 });
    expect(nextCell({ row: 2, step: 3 }, DIMS, "down")).toEqual({ row: 3, step: 3 });
    expect(nextCell({ row: 2, step: 3 }, DIMS, "up")).toEqual({ row: 1, step: 3 });
  });

  it("clamps at every edge — never wraps (spec law)", () => {
    expect(nextCell({ row: 0, step: 0 }, DIMS, "left")).toEqual({ row: 0, step: 0 });
    expect(nextCell({ row: 0, step: 0 }, DIMS, "up")).toEqual({ row: 0, step: 0 });
    expect(nextCell({ row: 5, step: 15 }, DIMS, "right")).toEqual({ row: 5, step: 15 });
    expect(nextCell({ row: 5, step: 15 }, DIMS, "down")).toEqual({ row: 5, step: 15 });
  });

  it("Home/End go to the row's first/last step, keeping the row", () => {
    expect(nextCell({ row: 4, step: 9 }, DIMS, "home")).toEqual({ row: 4, step: 0 });
    expect(nextCell({ row: 4, step: 9 }, DIMS, "end")).toEqual({ row: 4, step: 15 });
  });

  it("beat jumps move 4 steps and clamp at the ends", () => {
    expect(BEAT_STEPS).toBe(4);
    expect(nextCell({ row: 1, step: 2 }, DIMS, "beatForward")).toEqual({ row: 1, step: 6 });
    expect(nextCell({ row: 1, step: 14 }, DIMS, "beatForward")).toEqual({ row: 1, step: 15 });
    expect(nextCell({ row: 1, step: 2 }, DIMS, "beatBack")).toEqual({ row: 1, step: 0 });
    expect(nextCell({ row: 1, step: 3 }, DIMS, "beatBack")).toEqual({ row: 1, step: 0 });
  });

  it("single-step grids and single-row grids still behave", () => {
    const tiny = { rows: 1, steps: 1 };
    expect(nextCell({ row: 0, step: 0 }, tiny, "right")).toEqual({ row: 0, step: 0 });
    expect(nextCell({ row: 0, step: 0 }, tiny, "down")).toEqual({ row: 0, step: 0 });
    expect(nextCell({ row: 0, step: 0 }, tiny, "end")).toEqual({ row: 0, step: 0 });
  });
});
...
describe("lane moves", () => {
  ...
  it("carryCellTo clamps row + step into the target grid's shape", () => {
    ...
    expect(carryCellTo({ row: 5, step: 10 }, { rows: 7, steps: 16 })).toEqual({ row: 5, step: 10 });
    expect(carryCellTo({ row: 5, step: 10 }, { rows: 7, steps: 64 })).toEqual({ row: 5, step: 10 });
    expect(carryCellTo({ row: 5, step: 30 }, { rows: 7, steps: 16 })).toEqual({ row: 5, step: 15 });
    expect(carryCellTo({ row: 13, step: 3 }, { rows: 7, steps: 16 })).toEqual({ row: 6, step: 3 });
    expect(carryCellTo({ row: 2, step: -4 }, { rows: 7, steps: 16 })).toEqual({ row: 2, step: 0 });
  });
});
