...
the finishing phase of ultron-supreme iteration 7 (phone MIDI section rework,
auto mode: choice recorded as `simulated (auto mode)` — refresh). This
refresh captures iteration 7's shipped phone MIDI laws: the semitone-snap
register window (always exactly modeSize COMPLETE rows at every rest — boot,
steppers, wheel, scroll-end snap — at any semitone offset, the pane box
pinned to the renderer's inline height so a partial row can never intersect
it), the unified register controls (ONE OCT −/+ + ONE SEMI −/+ beside the
grid, bounds/readout from the MOUNTED pattern's manifest; the strip's
SOUND-transpose OCT moves into the phone OPTIONS drawer under the E9 fence
"changes what you HEAR, not what you SEE"), pitch-anchored notes (a note's
identity is its scale DEGREE — every window/scroll/zoom change re-maps runs
through the renderer seams, never a re-pitch; scale edits relabel in place),
pinch-to-zoom [1,2]× (live geometry re-fit through the renderer seams —
never a CSS transform — with a ≥44px factor chip + double-tap reset that
never eats the note under it), and the phone card header (the · EDIT word
hidden at phone scope; two edge-anchored nowrap tier rows — identity
LED+name left / MUTE+SOLO right, mix PRESET/KIT left / VOLUME right; one
line in landscape). As with iterations 2–6 this was a layout/interaction
iteration only: palette, typography, and corner language are untouched;
the frontmatter tokens are unchanged.
Prior refresh 2026-09-11 by $impeccable document (closing refresh, scan mode) for
...
- **Quadrant lane floor:** 4px radius, 1px 12%-ink border; selected = 55% lane-hue border + 3% hue fill tint + "· EDIT" text word (the word is hidden at phone scope — exactly one card is mounted there, so the marker is constant noise; the LED + name carry the card); view-only quadrants dim but keep every pair ≥ AA
...
- **Steppers:** 20px square 3px buttons with lane-hue border accent (35%), 7%-ink fill — the one stepper vocabulary serves preset stepping, the per-lane OCT −/+ transpose (popover-free in the lane header on desktop/tablet; at phone it lives in the OPTIONS drawer — one card, one octave control; one octave per press, clamped −3/+3, undoable, exports follow — the fence is explicit: OCT changes the octave a lane SOUNDS, the register row's OCT/SEMI and Shift+arrows scroll the octave you SEE), the phone register row's OCT/SEMI view steppers (44×44 painted at phone), and the pattern LENGTH ladder (BARS 1·2·4·8·16·32·64·128; grow always works, shrink refuses while a note would be lost past the new end, naming the blocking note)

### Navigation

- **Phone chrome (iteration 4):** centered PLAY/STOP transport flanked by the lane-switcher tabs and an OPTIONS trigger; the drawer that opens is the shared BoothOptions in the committed chassis vocabulary (6px floating-surface radius, chassis cast, internal scroll, Escape closes) and carries the ACTIVE lane's sound-transpose OCTAVE stepper (the E9 fence spoken where the control lives: "Changes what you HEAR, not what you SEE"), and the REGISTER row (ONE OCT + ONE SEMI steppers, the ROWS readout chip, the zoom chip) sits directly under the card header in the stepper language — one control voice, no phone-only idiom.
- **Phone card header (iteration 7):** TWO edge-anchored nowrap tier rows replace the wrapping compact strip — identity (LED + name LEFT at the card padding, MUTE + SOLO right at the trailing edge) over mix (PRESET/KIT stepper LEFT, VOLUME right-anchored), the edit tier (SCALE/GATE/FX) below with consistent left alignment; every control paints ≥44px and overflow SHRINKS through the flex chain (44px keys flex-shrink:0, value ellipsis past 26vw, slider floor 48px) — it never re-wraps into a third row. DOM order = visual order (tab order), and the desktop/tablet strip keeps the incumbent single-row child sequence byte-identically. At the rotated phone (≥768px wide inside phone scope) the two tier groups share ONE line (identity left / mix right-anchored), keeping the strip at its one-line height.
