...
- the compact single-line pattern rail;
...
**Stage classes (one `stageMode()` seam):**
- **Desktop (≥1024×600):** the 2×2 stage above.
- **Tablet (768–1024):** a responsive scale of the same 2×2 stage, still one page.
- **Phone (<768, or <1024 with height <600):** a single-lane stage.
  - The quadrant selection becomes a lane-switcher tab row inside one sticky chrome. The always-visible, centered PLAY/STOP key sits at its bottom.
  - Every other booth option (LOOP, METRO, VIZ, TEMPO, SCALE, SWING, MASTER) lives in a collapsible OPTIONS drawer that mounts zero DOM while closed.
  - KEYS and INFO do not render on phone.
  - The page scrolls; the chrome stays under 50% of the viewport height.

**Register windows.** Every pitched lane shows the same one-octave register window on every stage. The full row manifest is one scroll away: Shift+arrows on desktop, ±octave/±semitone shift rows on phone. Patterns longer than 64 steps render virtualized: a visible column window inside a pattern-wide scroll extent.

**Phone fill.** Pad rows fit the measured well exactly. Cell width = (well − label − (n−1)·gap)/n, clamped to 15–24px. Row tracks grow into the leftover height within 44–64px (44 is the finger floor), and the card bottom docks to the viewport bottom at scroll end.
...
- **Do** keep the register contract honest: OCT changes what a lane SOUNDS; window scroll changes what you SEE.
