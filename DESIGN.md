...
Creative effects live inside the selected-channel inspector on Mixer. Instruments retains quick level, mute and solo controls. Keep effect title, add, reorder, bypass, remove and parameter controls usable in empty and populated chains. Modules occupy compact 300px cards in one horizontal device rack, using the same quiet material and circular slider thumbs. The rack scrolls independently at every viewport; devices never become full-width vertical sections. Master includes the same editable effects rack.

Mixer uses one continuous bank of channel strips plus Master, with thin seams and horizontal scrolling when needed. Track colors identify the meters and the selected strip's top edge; a recessed fill and pressed state also mark selection. The Auto Mix controls appear above the strips, with a readable status line, proposed changes and explicit comparison/apply actions. The selected-channel inspector follows below. Preserve visible units, numeric readouts and labeled bypass states.

The Mixer heading uses Unbounded 500 at 24–34px; section and device headings use Barlow 600 at 20px and 17px. Numeric outputs use IBM Plex Mono. The page caps its width at 1440px. At widths up to 600px, strips are 176px wide, headings stack and processing controls use two columns. Phone and coarse-pointer buttons have 44px minimum targets; range controls and Auto Mix option labels have a 44px minimum height. Focus uses the existing accent outline. These are Mixer component measurements, not changes to the Membrane token system.
